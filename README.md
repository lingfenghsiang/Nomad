...
5. Setting up the environment.
   The global directory is set in file `global_dirs.sh`.
	...
	`MEMTIS_CXL_OPTION` is an option for Memtis. This option is used to select event and determine memory node. We hard coded the event and the memory node. Please always set it to `on`.
   ```
