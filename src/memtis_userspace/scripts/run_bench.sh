...
CGROUP_NAME="htmm"
###### update DIR!
DIR=/home/dws/memtis-benchmarks
...
function func_prepare() {
	...
	if [[ -e ${DIR}/bench_cmds/above_fast_mem/${BENCH_NAME}.sh ]]; then
	    source ${DIR}/bench_cmds/above_fast_mem/${BENCH_NAME}.sh
	else
...
function func_main() {
    ${DIR}/bin/kill_ksampled #also kill ssh 
    TIME="/usr/bin/time"
    ...
    sudo dmesg -c > ${DIR}/dmesg_debug.txt
    # disable htmm
