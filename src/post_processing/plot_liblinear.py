...
def plot_pr():
    pr_dir = glob.glob("src/tmp/results/liblinear*")
    cases = []
    data = []
    for i in pr_dir:
        cases.append(os.path.basename(os.path.normpath(i)))
        logname = os.path.join(i, "results-liblinear", "output.log")
    ...
    foo_fig = plt.gcf()
    foo_fig.savefig("src/post_processing/tmp/liblinear.png", bbox_inches='tight')
