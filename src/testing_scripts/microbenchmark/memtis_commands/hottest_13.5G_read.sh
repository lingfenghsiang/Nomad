...
function func_main() {
    ...
    mkdir -p ${DIR}/microbench_memtis/run-memtis-13.5g-read/microbench/
    LOG_DIR=${DIR}/microbench_memtis/run-memtis-13.5g-read/microbench/
...
mkdir -p ${result_dir}
BENCH_RUN="${compiled_package_dir}/tpp_mem_access -fwarmup=${compiled_package_dir}/warmup_zipfan_hottest_13.5G.bin -frun=${compiled_package_dir}/run_zipfan_hottest_13.5G.bin -fout=${result_dir}/zipfan_hottest_13.5G.read.log --logtostderr -sleep=5 -work=2"
func_prepare
func_main
