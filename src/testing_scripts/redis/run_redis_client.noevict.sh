...
mkdir -p ${output_dir}
${ycsb_dir}/bin/ycsb load redis -s -P src/testing_scripts/redis/workloada.${tag} -threads 10 -p redis.host=localhost -p redis.port=6379  -p redis.timeout=30000
...
(time ${ycsb_dir}/bin/ycsb run redis -s -P src/testing_scripts/redis/workloada.${tag} -threads 10 -p redis.host=localhost -p redis.port=6379 -p redis.timeout=30000 >> ${output_dir}/redis.noevict.log
